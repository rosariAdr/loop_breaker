...
describe('getPassiveModifiers', () => {
  ...
  it("Veteran's Resolve (maxHpBonus) — lv1 = 0.20, lv3 = 0.40", () => {
    expect(getPassiveModifiers([{ skillId: 'veterans_resolve', level: 1 }]).maxHpBonus).toBeCloseTo(
      0.2,
    )
    expect(getPassiveModifiers([{ skillId: 'veterans_resolve', level: 3 }]).maxHpBonus).toBeCloseTo(
      0.4,
    )
  })

  it('Stone Skin (damageReduction) — lv1 = 0.10, lv3 = 0.26', () => {
    expect(getPassiveModifiers([{ skillId: 'stone_skin', level: 1 }]).damageReduction).toBeCloseTo(
      0.1,
    )
    expect(getPassiveModifiers([{ skillId: 'stone_skin', level: 3 }]).damageReduction).toBeCloseTo(
      0.26,
    )
  })

  it('nouveaux passifs MON01 (stoneskin / thick_hide / mossy_hide)', () => {
    expect(getPassiveModifiers([{ skillId: 'stoneskin', level: 1 }]).damageReduction).toBeCloseTo(
      0.12,
    )
    expect(getPassiveModifiers([{ skillId: 'thick_hide', level: 1 }]).damageReduction).toBeCloseTo(
      0.08,
    )
  ...
  it('cumul de plusieurs passifs', () => {
    ...
    expect(mods.maxHpBonus).toBeCloseTo(0.2)
  ...
  it('ignore les ids inconnus / les skills actifs', () => {
    expect(getPassiveModifiers([{ skillId: 'nope', level: 1 }])).toEqual({
      maxHpBonus: 0,
      damageReduction: 0,
    })
    expect(getPassiveModifiers([{ skillId: 'savage_bite', level: 1 }])).toEqual({
      maxHpBonus: 0,
      damageReduction: 0,
    })
  })
