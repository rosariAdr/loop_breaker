    ...
    return (
      <div
        ...
        style={{
          ...
          background: 'radial-gradient(ellipse at center, #2a0808 0%, #0a0a0f 60%, #000 100%)',
        }}
      >
        <div
          className="flex flex-col gap-8 items-center text-center anim-pop"
          style={{ maxWidth: 640 }}
        >
          <p
            style={{
          ...
          <p style={{ color: '#c04040', fontSize: '1.4rem', fontFamily: 'var(--font-head)' }}>
            …has fallen.
          </p>
          <div
            className="py-6 px-6"
            style={{ borderTop: '1px solid #5a2020', borderBottom: '1px solid #5a2020' }}
          >
            <p
              style={{
                color: 'var(--gold)',
          ...
          <button
            className="pbtn"
            style={{ borderColor: '#5a2020', color: '#e04040', background: 'rgba(40,8,8,.6)' }}
      ...
      <div className="w-full flex flex-col gap-6" style={{ maxWidth: 640 }}>
        ...
        <div className="text-center">
          <p
            ...
            style={{
              color: 'var(--danger)',
              fontSize: '2.2rem',
              textShadow: '0 2px 18px rgba(192,57,43,.4)',
            }}
          >
            ☠ Fallen
          </p>
        ...
        <div className="pm-scroll flex flex-col gap-6">
          ...
          <div>
            ...
            >
              {statKeys.map((stat) => (
                <SummaryLine key={stat} label={stat} value={summary.stats[stat] ?? '—'} />
              ))}
            </div>
          </div>
          ...
          {showFirstDeathHint && (
            <div data-testid="first-death-hint" className="pm-hint anim-pop">
              ...
              <p style={{ color: 'var(--ink-soft)', fontSize: '0.82rem', lineHeight: 1.5 }}>
                Death is not the end. You may carry <strong>one stat</strong>,{' '}
                <strong>one active skill</strong> and
                <strong> one passive skill</strong> into your next life. Choose wisely — these
                become the foundation of a stronger run. The Gods' Shop awaits after.
              </p>
              <button
                className="pbtn"
          ...
          <div
            style={
              showFirstDeathHint
                ? {
                    boxShadow: '0 0 0 2px rgba(47,127,184,.3)',
                    borderRadius: 8,
                    padding: 14,
                    margin: -2,
                  }
          ...
          >
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {statKeys.map((stat) => (
                <button
                  ...
                  className={`pm-chip ${chosenStat === stat ? 'sel' : ''}`}
                >
            ...
            <div className="flex flex-col gap-1 mb-4">
              {activeSkills.length === 0 && (
                <button
                  ...
                  className={`pm-row ${chosenActive === null ? 'sel' : ''}`}
                >
              ...
              {activeSkills.map((s) => {
                ...
                return (
                  <button
                    ...
                    className={`pm-row ${chosenActive?.skillId === s.skillId ? 'sel' : ''}`}
                  >
            ...
            <div className="flex flex-col gap-1 mb-4">
              {passiveSkills.length === 0 && (
                <button
                  ...
                  className={`pm-row ${chosenPassive === null ? 'sel' : ''}`}
                >
              ...
              {passiveSkills.map((s) => {
                ...
                return (
                  <button
                    ...
                    className={`pm-row ${chosenPassive?.skillId === s.skillId ? 'sel' : ''}`}
                  >
                    {t?.name ?? s.skillId} · Lv {s.level}
                  </button>
                )
            ...
            <button
              ...
              disabled={!chosenStat}
              className="pbtn primary wide"
              style={!chosenStat ? { opacity: 0.45, cursor: 'not-allowed' } : undefined}
            >
              Enter the Gods' Shop →
            </button>

            {/* Reset complet — UX03 avec confirmation */}
            <button onClick={() => setPendingReset(true)} className="pm-reset">
