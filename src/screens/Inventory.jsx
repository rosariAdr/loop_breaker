    ...
    <div className="sheet-scrim" onClick={back}>
      <div className="sheet" onClick={(e) => e.stopPropagation()} style={{ width: 1080 }}>
        ...
        <div className="inv-tabs">
          {TABS.map((tab) => (
            ...
            >
              ...
              {tab === 'skills' &&
                `${getSkillContainer(CURRENT_UNIVERSE).icon} ${getSkillContainer(CURRENT_UNIVERSE).label} (${hero.inventory.manaStones.length})`}
        ...
        <div className="inv-content">
          ...
          {activeTab === 'resources' && <ResourcesTab resources={hero.inventory.resources} />}
        </div>
      </div>
    </div>
  )
...
function SkillsTab({ manaStones, hero, selected, setSelected, equipActive, equipPassive }) {
  ...
  const alreadyEquippedActive =
    selectedSkill && hero.activeSkills.some((s) => s.skillId === selectedSkill.skillId)
  const alreadyEquippedPassive =
    selectedSkill && hero.passiveSkills.some((s) => s.skillId === selectedSkill.skillId)
    ...
    <div className="flex gap-4">
      ...
      <div className="flex-1 flex flex-col gap-2">
        ...
        {manaStones.length > 0 && !selectedSkill && (
          <p className="inv-empty" style={{ fontSize: 12, marginBottom: 2 }}>
          ...
          return (
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <p className="inv-name" style={{ fontSize: 14 }}>
                  {t.name}
                </p>
                ...
                <span
                  className="inv-tag ml-auto"
                  style={{
                    background: t.type === 'active' ? 'rgba(185,122,18,.15)' : 'rgba(45,82,22,.13)',
                    color: t.type === 'active' ? 'var(--amber-deep)' : 'var(--forest-deep)',
                  }}
                >
                  {t.type}
                </span>
      ...
      {selectedSkill && template && (
        <div className="inv-detail">
          <p className="inv-name" style={{ fontSize: 16 }}>
            {template.name}
          </p>
          ...
          {template.type === 'active' && (
            <button
              onClick={() => {
                equipActive(selectedSkill)
                setSelected(null)
              }}
              disabled={!canEquipActive || alreadyEquippedActive}
              className={`inv-btn ${canEquipActive && !alreadyEquippedActive ? 'primary' : ''}`}
            >
              {alreadyEquippedActive
                ? 'Already equipped'
                : canEquipActive
                  ? 'Equip (Active)'
                  : 'Active slots full'}
          ...
          {template.type === 'passive' && (
            <button
              onClick={() => {
                equipPassive(selectedSkill)
                setSelected(null)
              }}
              disabled={!canEquipPassive || alreadyEquippedPassive}
              className={`inv-btn ${canEquipPassive && !alreadyEquippedPassive ? 'primary' : ''}`}
            >
              {alreadyEquippedPassive
                ? 'Already equipped'
                : canEquipPassive
                  ? 'Equip (Passive)'
                  : 'Passive slots full'}
  ...
  return (
    <div className="flex gap-4">
      ...
      <ConfirmDialog
        ...
        message={
          pendingSell
            ? `You are about to sell ${pendingSell.name} (${pendingSell.rarity}). This cannot be undone.`
            : ''
        }
        ...
        return (
          <div key={id} className="inv-card">
            <div className="flex justify-between items-center mb-1">
              <p className="inv-name" style={{ color: 'var(--forest-deep)', fontSize: 14 }}>
                {res.name}
              </p>
      ...
      {entries.map(([id, qty]) => {
        ...
        return (
          <div key={id} className="inv-card" style={{ borderLeft: `3px solid ${rarityColor}` }}>
            <div className="flex justify-between items-center mb-1">
              <p className="inv-name" style={{ color: rarityColor, fontSize: 14 }}>
                {res.name}
              </p>
              <span style={{ color: 'var(--ink)', fontSize: 14, fontWeight: 600 }}>×{qty}</span>
            </div>
            <p className="inv-sub">{res.description}</p>
            <p className="inv-sub" style={{ marginTop: 2 }}>
