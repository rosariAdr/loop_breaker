...
export default function CraftingMinigame({
  ...
  useEffect(() => {
    if (!open) {
      cleanup()
      setHits([]); setResolved(false); setCursorPct(0)
      return
    }
  ...
  const buttonLabel = mode === 'alchemy'
    ? '⚗️ STOP'
    : `🔨 STRIKE (${hits.length}/${totalStrikes})`
  ...
  return (
    ...
    >
      <div className="w-full max-w-md mx-4 rounded-xl p-6 anim-pop" style={{ background: '#0a0808', border: '1px solid #3a2818' }}>
        <p style={{ fontFamily: 'Cinzel, serif', fontSize: '1.05rem', color: '#d4af70', letterSpacing: '0.05em', marginBottom: '0.4rem' }}>
          {title ?? (mode === 'alchemy' ? 'Alchemy — Stop in the zone' : 'Forge — Strike the green zone')}
        </p>
        ...
        <div
          ...
          style={isVertical
            ? { width: '40px', height: '180px', margin: '0 auto', background: '#100808', border: '1px solid #2a1810' }
        ...
        >
          ...
          <div
            className="absolute"
            style={isVertical
              ? { bottom: `${zoneStart}%`, height: `${zoneSize}%`, left: 0, right: 0, background: 'linear-gradient(0deg, rgba(128,192,64,0.4), rgba(64,160,32,0.6))', borderTop: '1px solid #80c040', borderBottom: '1px solid #80c040' }
              : { left: `${zoneStart}%`, width: `${zoneSize}%`, top: 0, height: '100%', background: 'linear-gradient(180deg, rgba(128,192,64,0.4), rgba(64,160,32,0.6))', borderLeft: '1px solid #80c040', borderRight: '1px solid #80c040' }}
          />
          {/* Curseur */}
          <div
            ...
            className="absolute"
            style={isVertical
              ? { bottom: `${cursorPct}%`, height: '3px', left: 0, right: 0, background: '#ffd060', boxShadow: '0 0 8px #ffd060' }
              : { left: `${cursorPct}%`, width: '3px', top: 0, height: '100%', background: '#ffd060', boxShadow: '0 0 8px #ffd060', transform: 'translateX(-1.5px)' }}
          />
        </div>
        ...
        <button
          ...
          style={{ fontFamily: 'Cinzel, serif', background: '#1a1208', color: '#d4af70', border: '1px solid #5a3818', letterSpacing: '0.05em' }}
        >
          {buttonLabel}
        </button>
        {onCancel && (
