...
describe('Z07 — getLocationType', () => {
  it('ville reconnue quand currentLocation === city.id', () => {
    expect(getLocationType({ currentZone: 'ashenvale', currentLocation: ZONES.ashenvale.city.id })).toBe('city')
  })
  it('village par défaut sinon', () => {
    expect(getLocationType({ currentZone: 'ashenvale', currentLocation: 'millhaven' })).toBe('village')
    expect(getLocationType({ currentZone: 'ashenvale', currentLocation: 'inconnu' })).toBe('village')
  })
})
