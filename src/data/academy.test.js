...
describe('ACA01 — buySkill / sellSkill', () => {
  ...
  it("buySkill : assez d'or → mana stone ajouté + or débité", () => {
    useGameStore.setState((s) => ({
      hero: { ...s.hero, inventory: { ...s.hero.inventory, gold: 500, manaStones: [] } },
    }))
  ...
  it('buySkill : or insuffisant → refus, aucun changement', () => {
    useGameStore.setState((s) => ({
      hero: { ...s.hero, inventory: { ...s.hero.inventory, gold: 50, manaStones: [] } },
    }))
    const ok = store().buySkill('power_strike')
    ...
    useGameStore.setState((s) => ({
      hero: {
        ...
        inventory: {
          ...s.hero.inventory,
          gold: 0,
          manaStones: [{ skillId: 'power_strike', level: 3, xp: 0 }],
        },
      },
    }))
  ...
  it('sellSkill : skill non possédé → false', () => {
    useGameStore.setState((s) => ({
      hero: { ...s.hero, inventory: { ...s.hero.inventory, manaStones: [] } },
    }))
