...
- [ ] **PERF-IMG02 — Compression des assets binaires (MANUEL — asset, hors « Prêt à coder »)** (S, **P1 — avant 1er push**) — *issu de PERF-IMG01 (2026-06-10).* **Compresser les gros binaires** : `public/` map **9,7 Mo** + `rotting_shambler` ~5,9 / `gloom_bat` ~5,6 → **squoosh / TinyPNG**, cible **≤ 300–500 KB** chacun. ⚠️ **À faire AVANT le 1er push** (sinon bloat permanent de l'historique git). **Tâche d'outil d'image, pas de code** — ne peut pas être réalisée par édition de fichiers source ; à faire à la main.
