...
- [ ] **QSV2-LOCALITY01 — Quêtes appartenant à un lieu unique** (M, **P2**) — *modèle clarifié 2026-06-08.* **Chaque quête a un `issuedBy` = UN seul village/ville** (`quest.issuedBy ?? QUEST_NPCS[giverNpc].location`). Une quête de board est **disponible UNIQUEMENT à son lieu émetteur** (**pas** de partage par adjacence : 2 villages d'une **même zone PvE** ont des quêtes **distinctes**). Thématique : les quêtes d'un village portent sur les monstres de sa zone adjacente (contrainte de contenu, pas de dérivation). ⚠️ **Conséquence contenu** : aujourd'hui presque tout est émis par Ironhaven (donneurs `sir_aldric`/`captain`/`pell`) → Millhaven serait **vide** → **prévoir des quêtes propres à chaque village** (donneur par village ou `issuedBy` explicite). Fait évoluer GLD01/GLD02.
- [ ] **QSV2-TURNIN01 — Disponibilité & rendu par lieu** (M, **P2**) — *modèle clarifié 2026-06-08.* **Disponible** : seulement à `issuedBy`. **Rendu (Claim)** : au **village émetteur OU en ville** (la ville = point de rendu **universel** de toute quête acceptée). Pas besoin de tracker `acceptedAt` → tout se déduit de `issuedBy`. Filtres QuestBoard : `available` = `issuedBy === currentLocation` ; `active`/`completed` + bouton Claim = `issuedBy === currentLocation` **OU** ville. Remplace **GLD03**. + tests village/ville.
...
- [ ] **DEVBP02 — Contrats de composants (JSDoc / PropTypes)** (M, **P3**) — *issu de DEVBP01 item (3), sorti car orthogonal au formatage.* Documenter les props (JSDoc `@param` **ou** `prop-types` — pas de TS, cf. ADR-002). Cible prioritaire : composants réutilisables de `components/`, puis props publiques des screens. À cadrer (commencer par tout `components/` ?).
- [ ] **COV80 — Monter la couverture lignes à 80 %** (M, **P3**) — *issu de DEVBP01.* Seuil 80 % **configuré** mais réel = **71,06 %** → `npm run test:coverage` rouge par design (cible). Gap concentré sur les écrans : `SafeZone.jsx` 18 %, `WorldMapCanvas` 31 %, `WorldMap` 59 %, `Combat.jsx` 62 %, `PostMortem` 67 %, `Inventory` 69 %. Store/utils/engine déjà >90 %. Ajouter smoke tests RTL + flux sur ces écrans jusqu'à passer le seuil. **Lié à DX-CI01** (la CI doit pouvoir lancer `test:coverage` vert — sinon désactiver le `thresholds` en CI tant que COV80 n'est pas fait).
...
- [ ] **WM-LEVEL01 — Indication de niveau sous les noms (WorldMap)** (XS, **P2**) — *TC 2026-06-08.* Afficher la **fourchette de niveau** du spot **juste sous son nom** sur la WorldMap (ex. « Lv 1-8 » sous « Ashenvale Forest »), depuis `spot.levelRange`. Sous-label discret dans `WmNode`. Cohérent avec UX-MAPCLARITY01 + MONLV01 (niveau réel des monstres).
