...
| v1.31x (bugfix) | BUG_v1.31 — Correctifs playthrough (8) | passe de bugfix Quêtes (retours 2026-06-20) |
...
- [x] **QUI-QOBJ-STYLE01 — Prototypes de style pour la condition d'objectif (couleur/taille)** · S · P4 · 🟡 ✅ (fait — hybride retenu : libellé 14px/500 + compteur en pastille colorée par état ; appliqué QuestCard + QuestsOverlay, palettes sombre/parchemin ; tests UI mis à jour) — *cc Quests overlay (#2, design).* La ligne d'objectif `{label} (x/y)` manque de lisibilité. **Explorer 2-3 prototypes** : couleur de police selon l'état (en cours / prêt à rendre / fait ✓), police plus grosse, contraste sur parchemin. Livrer les variantes à comparer avant de trancher. AC : ≥2 variantes proposées + la retenue appliquée au `QuestCard` et à la Quests overlay. *(design, pas un bug strict.)*
