...
| v1.43 | Maîtres & mentors (12) | ✅ livré 12/12 (batchs 1-4) mergé dev 2026-07-04 · MST01-09 + décisions G1-3 + itinérants (MST08) · SKD-ICE01 + statut `frozen` · **board itinérant local** (un-lieu/4j, décision #3, batch 6) · ✅ réconcilié 2026-07-20 (FIX-ALDRIC01) : Aldric 100 % Greywatch, le `knight_trainer` de Millhaven = Dame Roswyn |
...
- [x] **TST-UI — Tests de non-régression UI (couverture cible ≥ 70 %)** · S · P2 · ✅ — **FAIT 2026-07-20 (branche `feat/v1.35_ui_critique`)** : passe `npm run test:coverage` verte (exit 0, seuil lines 80 du vite.config respecté) — **couverture lignes 84.83 %** (statements 82.67 %, functions 80.77 %), très au-dessus de l'AC ≥ 70 % ; ≥ 1 test par ticket corrigé (UI12 : 5 tests `WorldMap.ui12.test.jsx` ; UI11 : 9 tests `ChurchPanel.ui11.test.jsx` dont régression FIX-QUESTPROG02) ; **0 test cassé** (suite 3437 verte). Épique 🔴 URGENT close (3/3). — **Contexte** : couverture actuelle ~**76 %** ; UI12 (layout carte) et UI11 (panneau Église) touchent du layout et des composants Église → risque de régression. **Objectif** : maintenir la couverture **> 70 %** sur l'épique urgente (UI12 + UI11) via des tests ciblés.
...
- [x] **FIX-CRAFTSRC01 — Ingrédients sans source (bat_wing / void_fang / troll_blood)** · S · P2 · ✅ — **FAIT 2026-07-20 (branche `feat/v1.35x_bug_audit`)** : substitutions (option b) — mana potion `bat_wing→wolf_fang`, elixir minor `bat_wing→bone_fragment`, `void_fang→wolf_alpha_fang` (master_serpent_dagger + variantes forge serpent_dagger/swift_boots/hunters_grips ×8 combos), `troll_blood→verdant_ichor` (master_cursed_greaves) ; les 3 orphelines passent `uses:['sell']` (réserve DROP-FIX01) ; **test d'intégrité « obtainable » `craftObtainable.test.js`** (chaque ingrédient de chaque recette droppé par ≥1 monstre + les 3 morts bannis des recettes, écrit AVANT le fix : 11 combos rouges → 0). — 3 ingrédients `sources:[]` rendaient incraftables 2 recettes alchimie + 5 recettes de forge. **Hors scope** : rééquilibrage des taux. ⟶ QA01, craftRecipes.js.
- [x] **FIX-DIVSKILL01 — healing_bloom (divin) retiré de la vente Académie** · XS · P2 · ✅ — **FAIT 2026-07-20 (branche `feat/v1.35x_bug_audit`)** : retiré d'`ACADEMY_CATALOG` + 2 tests de régression (aucun skill `container:'divine'` au catalogue ; healing_bloom hors catalogue, `skillBuyPrice` null) écrits AVANT le fix. — `healing_bloom` est `container:'divine'` (option de Sylvara) mais figurait dans `ACADEMY_CATALOG` (academy.js:13, 200 or) alors que le catalogue et masters.js excluent les divins de la vente. ⟶ ADR-006 (esprit).
- [x] **FIX-ALDRIC01 — Réconciliation Aldric : un seul lieu** · M · P2 · ✅ — **FAIT 2026-07-20 (branche `feat/v1.35x_bug_audit`)** : le `knight_trainer` de Millhaven appartient à **Dame Roswyn** (PNJ dédié : tuile SafeZone, NpcOverlay fallback 🛡, cta « Train with Roswyn », dialogue `knight_roswyn`, buildingUnlocks) ; **l'onglet Quêtes du panneau a disparu** (il listait les quêtes Greywatch d'Aldric — elles vivent sur le board de Greywatch) → panneau = Techniques + Train + MasterBoard local (MST08 intact, Elyndra/itinérants) ; l'arbre `knight_aldric` reste en réserve pour la surface Greywatch d'Aldric (MST05) ; data canonique intacte (QUEST_NPCS/MASTERS location greywatch). **4 tests de régression `aldric01.test.jsx`** (écrits avant le fix) + KnightTrainerPanel/SafeZone.panels/itinerantVillage adaptés. Solde la note « ⚠️ à réconcilier » de v1.43. — ✅ **DÉCIDÉ (2026-07-19) : option (a).** ⟶ MST05.
- [x] **FIX-PLURAL01 — Pluralisation des noms de monstres (quêtes générées)** · S · P3 · ✅ — **FAIT 2026-07-20 (branche `feat/v1.35x_bug_audit`)** : champ optionnel `namePlural` (ashwood_wolf→Wolves, russet_fox→Foxes, thunderhoof→Thunderhooves, bone_colossus→Colossi future-proof) + helper `monsterPlural(monster)` (défaut régulier `name+'s'`, défensif) + 3 call sites `villageQuests.js` (kill name + label + patrol multi-objectifs). **Micro-test `plural.test.js`** (helper + scan anti-« Wolfs/Foxs » sur les pools générés, écrit avant le fix). Risque save : nul (ids stables). **Hors scope** : pluriels des ressources. ⟶ VQ02.
- [x] **FIX-LVLQUEUE01 — Modal Level Up unique, cumulé et reportable** · S/M · P3 · ✅ — **FAIT 2026-07-20 (branche `feat/v1.35x_bug_audit`)** : (1) **modal allocateur unique** — `pendingLevelUp` (N niveaux cumulés) devient N **points à répartir** via +/− par stat, compteur « X / N left », titre « +N levels » ; `commitLevelUp(allocations)` applique les points choisis et **reporte le reliquat** dans `hero.pendingStatPoints` ; (2) bouton **« Do it later »** (`deferLevelUp`) → reporte TOUS les points sans rien attribuer ; (3) **HeroSheet** : badge « ✦ Stat points to assign » + compte + boutons **+1 par stat** (`assignStatPoint`, décrémente le pool, badge disparaît à 0). **Migration save `pendingStatPoints`** (backfill 0, répare négatif/non-numérique). Tests : `lvlqueue.test.js` (coalescence, commit/defer/assign, migration) + `HeroSheet.lvlqueue.test.jsx` (badge + attribution) + LevelUpModal (screens.test.jsx) migré à l'allocateur. Suite complète verte + build OK. — cause racine : `pendingLevelUp` = simple compteur ; le modal n'accordait qu'1 point quel que soit N, et se rouvrait à chaque grant XP séparé. ✅ **DÉCIDÉ (2026-07-19).** ⟶ HeroSheet, LevelUpModal.
- [x] **FIX-SRCMON01 — Réconcilier 4 `sourceMonster` contradictoires + quasi-doublon stone_skin/stoneskin** · S · P3 · ✅ — **FAIT 2026-07-20 (branche `feat/v1.35x_bug_audit`)** : `stone_skin` + `venom_bite` → sourceMonster **null** (versions ENSEIGNÉES à l'Académie, jumelles ASSUMÉES des versions droppées `stoneskin` 12 %/`venom_strike` — commentées) ; `spectral_veil` → **null** (orpheline shop/futur, comme reckless_blow) ; `iron_resolve` → **barrow_wight** (son vrai droppeur, réserve). **Test de réciprocité `skillSources.test.js`** (écrit avant le fix : 4 paires rouges + **bonus détecté** soul_harvester/soul_crush = double droppeur légitime boss+réserve, règle assouplie en conséquence). AC tenus : chaque sourceMonster = vrai drop ; jumelles assumées et documentées. ⟶ SKD-E1, Codex.
- [x] **FIX-TOPBAR01 — Onglets Quests/Hero/Bag/Save rognés hors du fond topbar (investigation)** · S · P3 · ✅ — **INVESTIGUÉ 2026-07-20 (branche `feat/v1.35x_bug_audit`) → NON reproductible en 16:9 : artefact du ratio anormal, aucun correctif nécessaire.** Vérifié en jeu (dev server, mesures DOM) à **1920×1080 ET 1366×768** : `.topbar` remplit le canvas (`topbarFillsCanvas === 1`, aligné à gauche), **`.tb-tabs` à −18 px du bord droit (DANS le fond)**, `scrollWidth === clientWidth` (0 débordement H), **`clippedTabs: []`** (aucun onglet rogné) aux deux résolutions. Cause racine du rendu correct : le layout interne est fixé dans `.lb-canvas` (1920×1080) scalé uniformément (`min(vw/1920, vh/1080)`, App.jsx:92) → le topbar (enfant bloc) fait toujours la pleine largeur du canvas, spacer `flex:1` poussant les onglets à SON bord droit. L'observation initiale (990×438, scale 0.40 — ratio 2.26, hauteur extrême) était un **transitoire de redimensionnement** (pane redimensionné en continu, cf. mémoire), pas un bug 16:9. *Pas de test unitaire : jsdom ne calcule pas la mise en page ; la preuve est la mesure runtime ci-dessus.* ⟶ UI10, UI12, TST-UI.
- [x] **DX-LINTIGNORE01 — ESLint ignore `.claude/` + purge du worktree résiduel** · XS · P3 · ✅ — **FAIT 2026-07-20 (branche `feat/v1.35x_bug_audit`)** : `.claude/**` ajouté à `globalIgnores` (eslint.config.js) → **`npm run lint` vert sur tout le repo (exit 0)** ; worktree `jovial-gagarin-7b4347` **désenregistré** de git (`git worktree remove` + prune ; branche `feat/v1.44_assets` et son commit préservés). ⚠️ Le répertoire physique était verrouillé par un processus actif au moment du fix → il reste sur disque (cosmétique, à supprimer manuellement après libération du verrou) ; git ne le référence plus et le lint l'ignore. AC (lint vert) tenu.
- [x] **FIX-ZONELIST01 — Purger `ZONES.ashenvale.monsters` (liste morte pré-MON01)** · XS · P4 · ✅ — **FAIT 2026-07-20 (branche `feat/v1.35x_bug_audit`)** : les 3 listes `monsters` **zone-level** (ashenvale + blighted_road + grimspire, legacy sans consommateur runtime, ashenvale contenait 4 ids morts) RETIRÉES ; source de vérité = MONSTERS_BY_ZONE/_SPOT + les listes `monsters` des huntingSpots (valides). **Garde-fou `zones.deadids.test.js`** (chaque monstre de huntingSpot existe dans MONSTERS + zone-level absente). AC tenus.
...
- [x] **REL-CHANGELOG01 — Rattrapage CHANGELOG + coupe `[Unreleased]` → `[0.2.0]`** · S · P2 · ✅ — **FAIT 2026-07-20 (branche `feat/v1.36_release`)** : `docs/CHANGELOG.md` — nouvelle section datée **`## [0.2.0] - 2026-07-20`** (Added v1.42/v1.43/onboarding + UI critique ; Changed v1.34 + i18n ; **Fixed** = les 13 tickets BUG_AUDIT-0717 ; Process TST-UI/TEST-SCEN01) ; `[Unreleased]` vidé ; l'ancien contenu v1.31→v1.33 roulé sous 0.2.0. AC : version 0.2.0 datée prête pour le tag. *(Purge fine « sections déjà dans master » : master n'a aucun tag à ce jour → tout le delta va dans 0.2.0.)*
- [x] **REL-CONTEXT01 — Refresh CONTEXT.md (systèmes + compteurs §5)** · S · P2 · ✅ — **FAIT 2026-07-20 (branche `feat/v1.36_release`)** : compteurs corrigés depuis les vraies valeurs — **50 skills (6 divins)**, **4 spots surface** (SPOT_OWNER), **craftRecipes.js source unique** (21 autorées + variantes ≈ 39 ALL), **4 titres**, tomes en ville, **2044 tests / 178 fichiers** (⚠️ **correction majeure** : l'ancien « 3415/301 » était gonflé par le worktree résiduel doublonné — vrai total = 2044/178, cf. DX-LINTIGNORE01), bundle ~160 KB gz, statut → release 0.2.0. AC tenu. *(Refresh ciblé des compteurs/statut ; la narration détaillée §3-4 reste à jour au fil de l'eau.)*
- [x] **REL-TRAIN01 — Merge `feat/onboarding_quests` dans le train 0.2.0** · XS · P2 · ✅ — **EMBARQUÉ** : l'onboarding est déjà dans l'historique de `dev` (merges `feat/onboarding_quests` 6b6f9d5/9bcc0c8) et **traduit en anglais** (FIX-I18N01 ✅), donneuse ré-affectée à Elder Moira + arme `worn_iron_dagger` (commit 5ed07a2). Suite verte. AC tenu.
- [~] **TEST-SCEN01 — Recette fonctionnelle = gate sine qua non** · S · P2 · 🟡 — **(1) FAIT 2026-07-20** : CONTRIBUTING **§7** amendé — la « recette fonctionnelle 10/10 (date+commit) » est désormais **étape explicite sine qua non** de la PR `dev → master` (+ resync `dev` + refresh compteurs). **(2) RESTE À ADRIAN** : dérouler la **passe 10/10** de `docs/TEST_SCENARIOS.md` **en jeu réel** (S2-S5, S7, S10 + compléments) — c'est un gate **manuel/navigateur** (les tests unitaires 2044✓ ne le remplacent pas). À faire juste avant d'ouvrir la PR de release.
- [~] **REL-TAG01 — Bump 0.2.0 + PR dev→master + tag + resync** · XS · P2 · 🟡 — **(1) FAIT 2026-07-20** : `package.json` **0.0.0 → 0.2.0** (branche `feat/v1.36_release`). **(2) RESTE À ADRIAN** (« pas de merge, je m'en occuperai ») : merger les branches `feat/v1.35_ui_critique` → `feat/v1.35x_bug_audit` → `feat/v1.36_release` dans `dev`, PR `dev → master` (merge classique), `git tag v0.2.0` + push --tags, resync `dev` (`--ff-only`, check `git log master..dev` vide), puis DEPLOY01 hors-repo (push + import Vercel + **Vercel Authentication**). ⚠️ **Push bloqué côté agent** (pas de credential GitHub en session non-interactive) → toutes les branches sont **committées localement**, à pousser via GitKraken.
...
- [x] **QSV2-DROPDUP01 — Retirer les quêtes legacy doublonnées** · S · **P2** · ✅ — **FAIT 2026-07-20 (branche `feat/v1.35x_bug_audit`)** : `first_blood` + `nc_oakheart_elite` retirées de `quests.js` + `questBalance.js` ; **migration save** dans `normalizeSave` — les ids de quête inconnus sont filtrés de `activeQuests`/`completedQuests`/`questProgress` (les quêtes vq_*/église/maître/MQ/onboarding résolues par `getQuestById` survivent) ; test de régression `qsv2Dropdup.test.js` (7 tests, écrit AVANT le fix) + ~15 fichiers de tests migrés sur mq01_waking/nc_thin_the_boars/nc_thunderhoof_elite. — *P3→P2 (2026-07-19) : doublon First Blood ↔ mq01 **confirmé à l'écran** au board de Greywatch — rejoint le train 0.2.0.* ✅ **DÉCIDÉ (2026-07-11) : drop.** Post-re-domiciliation, `first_blood` (5× ashwood_wolf) **duplique `mq01`** et `nc_oakheart_elite` (Old Oakheart) **duplique `mq02`** au **même** village (Greywatch). Les **supprimer** de `quests.js` (+ registre `QUEST_NPC_REGISTRY`/résolution) ; **migration save** (une quête retirée encore `active`/`completed` ne doit pas casser `getQuestById`/le board — filtrer les ids inconnus) ; test de régression. *(`proof_of_worth` « atteindre Lv3 » : conservée — pas un doublon de cible.)* ⟶ QSV2-ADJ-AUDIT01, MST03.
- [x] **QSV2-QSRC-TEST01 — Test de sourcing & disjonction des quêtes** · S · **P2** · ✅ — **FAIT 2026-07-20 (branche `feat/v1.35x_bug_audit`)** : `questAdjacency.test.js` étendu (6 tests : owner unique par spot, mapping complet, owner ⊂ adjacence, 0 doublon inter-village par cible ET par id, sourcing sur spots possédés, garde-fou hand-authored). **Bonus : le test a détecté un vrai double-dip** — `collect:ectoplasm` généré à Millhaven ET Ironhaven (ressource partagée briar_wraith/ruin_specter) → fix `resourceOwner()` dans `villageQuests.js` (une ressource partagée n'est proposée que chez le propriétaire du spot de sa 1ʳᵉ source). — *P3→P2 (2026-07-19) : train 0.2.0 avec DROPDUP01.* — étendre `questAdjacency.test.js` : (1) **disjonction** — aucun spot n'alimente 2 lieux (via `SPOT_OWNER`) ; (2) **0 doublon inter-village** — deux lieux ne proposent jamais la même cible ; (3) **sourcing** — toute quête de village cible un monstre/drop d'un spot **possédé** par son émetteur ; (4) les quêtes hand-authored restent conformes (test d'adjacence existant vert). ⟶ QSV2-SPOTOWNER01, QSV2-DROPDUP01.
...
- [x] **QA-BOOKS01 — Les livres `tome_of_*` ne sont vendus/donnés nulle part** · XS · P4 · ✅ — **FAIT 2026-07-20 (branche `feat/v1.35x_bug_audit`)** : les 3 tomes (Focus/Might/Wisdom) vendus par le marchand en **majeure cité uniquement** (`bookStock` gated `getLocationType==='city'`, rendus à la suite des potions, réutilisent `buyPotion`/`addConsumable`). **4 tests `MerchantPanel.test.jsx`** : intégrité (tout `isBook` ∈ stock ville), présence ville, absence village, achat (or débité + livre au sac). ⟶ QA01, ITM01.
...
- [ ] **CMB-DIR01 — Direction artistique & présentation du combat (maquettes avant code)** · S/M · P3 · 🟡 — *demande 2026-07-19 : le combat actuel « pas glamour, pas assez stratégique » visuellement.* Passe de design AVANT toute implémentation ATB, en 3 temps : (1) **recherche de références** (Claude) — analyse visuelle des meilleurs combats 2D tour par tour (Slay the Spire, Sea of Stars, Octopath, Darkest Dungeon II, Chained Echoes…) : composition de l'arène, lisibilité timeline/intents, feedback des coups ; (2) **exemples d'Adrian** — captures/jeux qu'il aime, versées au moodboard ; (3) **maquettes itératives en artifacts** (pattern wm-ratio-previews / proto forge : on tranche sur du visible) → **livrable = maquette cible validée** de l'écran de combat (arène + timeline + intents + actions), déclinable ensuite en tickets d'implémentation + prompts assets Gemini (fonds d'arène, cadres, VFX — manifeste asset_forge). **Hors scope** : tout code de jeu ; les sprites de monstres (CONT01). Contrainte : rester cohérent avec l'identité « takeover sombre + Cinzel + or » actée en UI09, ou la faire évoluer explicitement. AC : ≥2 directions proposées, 1 validée par Adrian, maquette cible annotée archivée dans docs/. ⟶ CMB-ATB02, UI09, asset_forge, ART-EQUIP01.
  - **Phases 1-2 ✅ (2026-07-21)** — analyse de références livrée : `docs/design/CMB-DIR01-references.md`. Références retenues par Adrian : **Darkest Dungeon** (placement des ennemis, disposition skills/actions, stats du perso joué) + **Chained Echoes** (disposition des personnages, fenêtre d'action, file des tours haut-droite). Octopath écarté. Captures : `design/reference/screenshots/combat/`.
  - **⚠️ DÉCISION AD À ACTER — bascule chaude (évolution explicite de UI09)** : direction demandée = *heroic medieval fantasy, léger, magique, nature & courage, couleurs chaudes plutôt que sombres*. Le combat **rejoint** la palette parchemin/forêt/or déjà en place plutôt que de rester dans le registre takeover sombre (`--color-bg #0a0a0f`). Cinzel + or conservés. **Portée à trancher** : combat seul, ou aussi GodsShop/DivineCall/PostMortem ? (DivineCall gardé sombre/mystique en UI09.)
  - **Contrainte structurante retenue** : composition **multi-personnages dès maintenant** (3-4 emplacements alliés réservés) en prévision des compagnons (v1.61) — chaque direction maquettée en variante 1 héros ET 4 personnages.
  - **✅ DÉCISION AD #2 (2026-07-21, Adrian)** — la **bascule chaude ne concerne QUE l'écran de combat**. GodsShop / DivineCall / PostMortem **restent sombres** : ce sont des écrans d'après-mort, le registre sombre y est justifié diégétiquement. Les barres de vie peuvent être déplacées si la composition l'exige (contrainte levée).
  - **Phase 3 ✅ (2026-07-21)** — 2 directions maquettées, cliquables, à l'échelle 1920×1080, en variantes 1 héros ET 4 personnages : `docs/design/CMB-DIR01-mockups.html` (publié en artifact). **A « Le rang »** (arène latérale sur cour de pierre, barres au sol, stats du perso actif permanentes, bandeau de tours pleine largeur — console 37 % de la hauteur) vs **B « La clairière »** (décor peint lumineux, file des tours en portraits haut-droite, fenêtre d'action en grille à coûts alignés, statut party en tableau numérique — scène 72 %). **Arbitrage à rendre par Adrian.**
  - **✅ ARBITRAGE RENDU (2026-07-21, Adrian) — direction A « Le rang » RETENUE**, avec une demande d'évolution : **fond peint par zone** (plus la cour de pierre neutre) **et rendu différent en rencontre élite**. Maquette mise à jour : sélecteur de zone (4 spots de chasse réels + donjon) × rencontre (commun / élite).
  - **Conséquences actées** : (a) **A devient asset-gated** — c'était son principal avantage de ne pas l'être ; 5 fonds sont désormais bloquants (`ASSET_PROMPTS §11` promu 🔴 REQUIS) ; (b) **l'élite est un traitement CSS** (ciel assombri + braises + vignette rouge) sur la même plaque peinte → **5 illustrations au lieu de 10**, réversible spot par spot ; (c) la composition change aussi (**3 adversaires alignés → 1 adversaire surdimensionné**) ; (d) **barres au sol renforcées** (plaque opaque + liseré clair) — elles ne tenaient que grâce au fond neutre.
  - **Phase 4 ✅ (2026-07-21)** — kit découpé en **5 composants** (`ArenaBackdrop`, `Combatant`, `TurnBand`, `ActiveCharacter`, `ActionGrid`) dans `design/combat/` (jsx + d.ts + prompt.md, convention du projet) → **poussés dans le Design System** (`components/combat/`, 17 fichiers, carte `@dsCard` « Combat · arena, turn band, action console »). Styles additifs `combat.css` (préfixe `lb-cbt-`, **ne touche pas** `lb-components.css`). **Maquette cible annotée** : `docs/design/CMB-DIR01-target.md`.
  - **➡️ CMB-DIR01 CLOSABLE** — AC remplies : ≥2 directions ✅ · 1 validée ✅ · maquette cible annotée archivée dans `docs/` ✅.
  - **⚠️ Chemin critique = les assets, pas le code.** Les 5 fonds d'arène (§11) bloquent la livraison de l'écran.
  - **✅ ARBITRAGE RENDU (2026-07-21, Adrian)** — **les 5 fonds d'abord, puis l'écran complet livré d'un coup**. Pas de livraison intermédiaire avec les dégradés CSS. Séquence : `ASSET-ARENA01` → refonte de `Combat.jsx` sur le kit `design/combat/`.

- [ ] **ASSET-ARENA01 — 5 fonds d'arène peints** · S · P3 · 🟡 — *dérivé de CMB-DIR01 (2026-07-21).* Générer les 5 illustrations 16:9 `public/arenas/` : `ashenvale_forest`, `wildmere_hills`, `crumbled_ruins`, `thornmarsh`, `hollow_crypt`. **Prompts prêts à coller** dans `ASSET_PROMPTS` §11 (promu 🔴 REQUIS). Pas de variante élite à générer : le rang est un **calque CSS** sur la même plaque. AC : 5 PNG en place, contraste des barres au sol et des intents revalidé sur chaque fond (clair ET sombre). **⟶ bloque la refonte de l'écran de combat.**
